...
const RetryConditions = {
  ...
  rateLimitErrors: (error) => {
    return error.code === 'PLATFORM_RATE_LIMIT' ||
  ...
  temporaryErrors: (error) => {
    return error.code === 'PLATFORM_UNAVAILABLE' ||
...
class PerformanceMetrics {
  ...
  getSummary() {
    ...
    return {
      ...
      avgListingsPerPlatform: this.metrics.listings.total / Object.keys(this.metrics.platforms).length
    };
  }
...
const MONITORING_ALERTS = {
  ...
  HIGH_ERROR_RATE: {
    ...
    severity: 'error',
    message: (metrics) => 
      `${metrics.errors} errors occurred during run`,
  ...
  NOTIFICATION_FAILURE: {
    ...
    severity: 'error',
    message: (metrics) => 
      `${metrics.notifications.failed} notification(s) failed to send`,
...
class GracefulDegradation {
  ...
  async handlePlatformFailure(platform, error, remainingPlatforms) {
    ...
    if (failureCount >= 3) {
      ...
      Actor.log.error(`Platform ${platform} has failed 3 consecutive times, disabling for 24 hours`);
    }
...
class EbayScraper extends BaseScraper {
  ...
  hasAuthenticityGuarantee(rawListing) {
    ...
    return subtitle.includes('Authenticity Guarantee') || 
           title.includes('Authenticity Guarantee');
  }
...
describe('Notification Integration', () => {
  test('should send email notification', async () => {
    ...
    const result = await notifier.sendAlert(
      ...
      { searchTerms: ['Test'], platforms: ['Test'] }
    );
...
This comprehensive component specification document provides detailed technical specifications for all major components of the SneakerMeta Apify actor, including:
...
This specification is sufficiently detailed that developers can implement the actor without ambiguity. Each component has clear interfaces, data structures, error handling, and test requirements.
...
*End of Component Specification Document*
